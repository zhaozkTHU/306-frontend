...
const TagList: React.FC = () => {
  ...
  const fetchTasks = async () => {
    const response = await fetch('/labeling', { // undetermined
      method: 'GET',
      headers: {
  ...
  if (task.template === 'ImagesClassification') {
    ...
    return (
      <Modal
        ...
        footer={null}
      >
        <TextClassificationComponent title={task.title} create_at={task.create_at} deadline={task.deadline} template={task.template} 
        reward={task.reward} time={task.time} labeler_num={task.labeler_num} demander_id={task.demander_id} task_data={task.task_data} />
      </Modal>
    );
  }
    console.log(task);
