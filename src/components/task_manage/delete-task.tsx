...
const DeleteTask: React.FC = () => {
	...
	useEffect(() => {
		...
		request(`/api/task/demander_id?${userId}`, "GET")
			.then((value) => { setTaskInfo(value.demander_tasks); })
			.catch((reason) => { console.log(reason); message.error("获取任务失败"); })
	...
	const onDelete = (taskId: number) => {
		...
		request(`/api/task/task_id?${taskId}`, "DELETE")
			.then((_) => { setDeleteNum((num) => num + 1); message.success("删除成功"); })
			.catch((reason) => { console.log(reason); message.error("删除任务失败"); })
			.finally(() => setLoading(false));
	};
